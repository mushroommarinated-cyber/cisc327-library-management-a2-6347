...
def test_patron_status_report_example_data_structure():
    """Check the structure of borrowed books data."""
    report = get_patron_status_report("123456")
    books = report.get("books_currently_borrowed", [])
    for book in books:
        assert "title" in book
        assert "due_date" in book

def test_patron_status_report_empty_for_new_patron():
    """Check empty lists and zero fees/count for a new patron."""
    report = get_patron_status_report("999999")  # guaranteed no activity
